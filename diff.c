...
void version(void) {
  ...
  printf("Written by Colin Hughes\n");
}


int files_equal(void) {
  /*size_t n1 = sizeof(strings1) / sizeof(strings1[0]);
  size_t n2 = sizeof(strings2) / sizeof(strings2[0]);
  if (n1 != n2) {return 0; }
  for(int i = 0; i < n1; i++) {
    if (strcmp(strings1[i],strings2[i]) != 0) { printf("FILES NOT EQUAL\n"); return 0; }
  }
  printf("FILES EQUAL\n");
  return 1;*/

  return ( para_fileequal(strings1, count1, strings2, count2) );
}

void brief(const char* filename1, const char* filename2) {
  if (files_equal() == 0) { printf("Files %s and %s differ\n", filename1, filename2 ); } //that's it
}

void identical(const char* filename1, const char* filename2) {
  if (files_equal() == 1) { 
    printf("Files %s and %s are identical\n", filename1, filename2 ); 
    exit(0); //leave if identical files, do nothing otherwise
  } //little bit more
}
...
//  para_printfile(strings2, count2, printright);
  ...
  while (p != NULL) {
    ...
    if (foundmatch) {
      ...
      para_printboth(p, q); //custom function to print both
      p = para_next(p);
      q = para_next(q);
    } else {
